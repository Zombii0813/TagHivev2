...
- 修复出现的标签拖拽功能问题，并进一步优化功能效果
- 修复删除标签后原标签所关联文件卡片中的标签印记变为灰色但不消失的问题
...
- 标签栏功能扩展
  - 原标签拖拽排序取消
  - 增加标签管理模式，可进行标签列表自由排序
- 标签层级体系扩展
  - Tag 表新增 `parent_id` 字段，支持标签树形嵌套
  - TagPanel 侧边栏展示树形折叠标签结构
  - 选中父标签时，自动包含所有子标签匹配的文件
- 新增多工作区联合视图功能
  - 支持启用多目录作为工作区
  - 多工作区支持“全局视图”实现全局搜索
  - 工作区之间可共享全局标签
...
- 拖拽功能优化
  - 标签拖拽显示效果优化
- 生产版本pyInstaller构建所有依赖展开为目录，提升启动速度
- 文件夹树浏览板块优化
  - 列表模式更换为蜂巢模式
  - 优化为悬停图标，减小占用空间，鼠标悬停上方时可展开
- 标签过滤功能优化
  - 在目录蜂巢树模式下标签过滤仅显示所在子目录中的文件
